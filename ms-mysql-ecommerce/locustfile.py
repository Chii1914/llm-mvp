        ...
        for _ in range(3):
            ...
            with self.client.post("/products", json=payload, headers=self.headers, catch_response=True) as resp:
                if resp.status_code in (200, 201) and resp.text:
                    try:
                        data = resp.json()
                        prod_id = data.get("id_producto") or data.get("id") or data.get("_id")
                        if prod_id:
...
# NOTE: Run Locust in the project folder (ms-mysql-ecommerce) with:
#   pip install locust
#   locust -f locustfile.py --host=http://localhost:3002
# Or headless:
#   locust -f locustfile.py --host=http://localhost:3002 --headless -u 10 -r 2 --run-time 5m
